# DentalStock 작업 보고서

작성일: 2026-05-20

## 버전 관리

- 현재 작업 전 상태는 `codex-current-snapshot` 브랜치에 `chore: snapshot current dentalstock work` 커밋으로 보존했습니다.
- 수정 작업은 `codex-fix-user-test-issues` 브랜치에서 진행했습니다.
- `.claude/`, `COMMIT_MESSAGE.txt`, `COMMIT_MSG.txt`, `COMMIT_MSG_VENDOR.txt`는 임시 파일로 판단해 커밋 대상에서 제외했습니다.

## 주요 수정

### 발주/배송 플로우

- pending 발주에서 바로 `추적 시작`으로 ordered 상태가 되는 우회 경로를 제거했습니다.
- 관리자/원장에게만 `승인`, `반려`, `송장 등록`, `입고 확인` 액션을 노출하도록 정리했습니다.
- 위생사는 자기 발주 요청을 볼 수 있지만 승인/송장 등록/입고 확인은 할 수 없도록 UI와 훅 레벨에서 모두 막았습니다.
- 승인 후에는 진행중 탭에서 `송장 등록`을 할 수 있고, 송장 등록 후 `송장 상세`를 볼 수 있게 했습니다.
- 반려된 발주를 확인할 수 있도록 배송추적에 `반려` 탭을 추가했습니다.

### 오버레이/모달

- 품목 상세 화면에서 발주 요청 시트를 열면 뒤쪽 버튼이 클릭을 가로채던 z-index 문제를 수정했습니다.
- `BottomSheet`가 상세 오버레이보다 위에 오도록 정리해 실제 `발주 요청 보내기` 버튼이 정상 클릭됩니다.

### 날짜 처리

- `todayKey()`가 UTC 날짜를 쓰던 문제를 고쳐 `Asia/Seoul` 기준 날짜를 사용하도록 변경했습니다.
- 홈의 오늘 수술 카드와 입출고의 오늘/어제 표시가 한국 날짜 기준으로 동작합니다.

### 홈/문구 정리

- 홈의 `주문 승인 대기`, `배송 완료`, `주문 링크`처럼 실제 상태와 어긋나던 문구를 정리했습니다.
- pending은 `발주 승인 대기`, ordered는 `입고 대기`로 표시합니다.
- 가격이 없는 항목은 `0원` 대신 `가격 미등록`으로 표시합니다.

### 입출고 UX

- 입출고 품목 선택 시트에 `품목명 검색` 입력을 추가했습니다.
- 현재 재고보다 많은 출고 수량을 입력하면 `재고 부족` 상태로 제출 버튼을 비활성화합니다.
- 훅에서도 과출고를 실제로 막아 재고가 임의로 0 처리되는 일을 없앴습니다.

### 관리 화면

- `품목 추가`, `초기 재고 일괄 입력`, `초기 데이터 리셋`을 직원 관리에서 빼고 `품목 관리` 탭으로 분리했습니다.
- 도매 설정에서 사이트별 ID/비밀번호 입력 필드를 추가했습니다.
- 도매 설정 안내 문구를 현재 로컬 저장 기반의 테스트용 설정임을 알 수 있게 조정했습니다.

### 배송 상세

- 가격 데이터가 없는 주문에서 `원`만 표시되거나 `undefined원`처럼 보이는 문제를 제거했습니다.
- 배송 타임라인 더미 상태를 ordered/received 상태에 맞춰 자연스럽게 정리했습니다.
- 화면 텍스트에 섞여 있던 불필요한 기호성 표시를 줄였습니다.

## 검증 결과

- `npm run lint`: 통과
- `npm test`: 13개 테스트 파일, 127개 테스트 통과
- `npm run build`: 통과
- Playwright 실제 브라우저 플로우 검증: 최신 UX 7개 체크 + 최종 회귀 21개 체크 통과

## 실제 브라우저 검증 항목

- 원장 로그인 후 오늘 수술 카드가 한국 날짜 기준으로 노출되는지 확인
- 홈에서 pending/ordered 주문 문구가 실제 상태와 맞는지 확인
- pending 발주에서 `추적 시작` 우회가 사라졌는지 확인
- 관리자 승인 후 송장 등록과 송장 상세가 정상 동작하는지 확인
- 배송 상세에서 결측 가격과 타임라인 표시가 깨지지 않는지 확인
- 위생사 계정에서 승인/송장 등록 권한이 노출되지 않는지 확인
- 품목 상세에서 발주 요청 시트가 실제로 클릭 가능한지 확인
- 입출고 품목 검색과 과출고 차단이 동작하는지 확인
- 품목 관리 탭과 도매몰 계정 입력 필드가 노출되는지 확인

## 추가 개선

사용자 요청에 따라 버그 수정 외에도 한 번 더 제품 관점의 개선을 진행했습니다.

- 하단 내비게이션과 헤더의 `배송추적` 명칭을 실제 기능 범위에 맞춰 `발주/배송`으로 변경했습니다.
- 발주/배송 탭 배지를 pending만이 아니라 pending/ordered 활성 업무 전체 기준으로 계산하도록 개선했습니다.
- 홈의 `오늘 해야 할 일` 카드는 그룹별 반복 문구 대신 실제 개별 업무 목록처럼 보이도록 재구성했습니다.
- 홈 업무 카드의 건수는 그룹 개수가 아니라 실제 처리할 작업 수를 표시합니다.
- 출고 화면의 증가 버튼과 빠른 수량 버튼 색상을 입고/출고 맥락에 맞춰 파란색/빨간색으로 구분했습니다.
- 관련 테스트를 새 UX 기준으로 갱신했고, 동일한 125개 테스트 전체 통과를 재확인했습니다.

## 마지막 코드 다이어트

자기 전 맡긴 최종 정리 요청에 맞춰 기능은 유지하고 코드 반복과 불필요한 연결을 줄였습니다.

- `ShippingOrderCard`의 반복되던 카드 본문, 상태 헤더, 액션 버튼 스타일을 작은 공통 컴포넌트/상수로 정리했습니다.
- 발주/배송 화면의 권한별 주문 표시 기준을 `canApprove` 하나로 통일해 역할 조건 중복을 제거했습니다.
- 입출고 시트의 입고/출고 색상 분기를 `accent` 값으로 모아 같은 삼항식이 여러 번 반복되지 않게 했습니다.
- 실제로 쓰지 않는 `AdminScreen`, `OrderRequestSheet`, `ReceiptConfirmSheet` props 전달을 제거했습니다.
- 최종 정리 후 `npm run lint`, `npm test`, `npm run build`, Playwright 21개 실제 흐름 검증을 모두 다시 통과했습니다.

## Toss 디자인 시스템 적용

제공된 Toss/TDS 문서 기준으로 현재 앱에 적용 가능한 디자인 디테일을 새 브랜치에서 반영했습니다.

- Toss 토큰 값을 문서 기준에 맞춰 조정했습니다: `blue600`, `blue50`, warm grey scale, green/orange/yellow/teal/purple semantic 색상.
- 카드, 배지, 입력, segmented tab, toast, bottom navigation의 radius, shadow, padding, font weight를 TDS Mobile 규칙에 맞게 다듬었습니다.
- 재고 수량, 입출고 수량, 통계 값, 분석 숫자 등 데이터성 숫자에 monospace + tabular numeral 표현을 적용했습니다.
- 카테고리/탭 선택처럼 사용자가 누르는 인터랙션은 Toss Blue 중심으로 정리하고, red/orange/green은 상태와 위험 의미에 집중하도록 조정했습니다.
- 입출고, 재고 상세, 알림, 배송, 홈 업무 카드, 관리자 분석 화면의 버튼과 요약 카드 밀도를 모바일 375px 기준에 맞게 재정렬했습니다.
- 적용 후 `npm run lint`, `npm test`, `npm run build`, Playwright 21개 실제 흐름 검증을 모두 통과했습니다.

## 후속 UX 보완

사용자가 추가로 짚은 실제 사용 불편 사항을 반영했습니다.

- 입출고 등록에서 품목 선택 시트의 제목/X/검색 영역을 리스트 스크롤 영역 밖으로 분리해, 스크롤 중에도 상단 닫기 기능이 사라지거나 잘리지 않도록 고정했습니다.
- 입출고 화면의 `입고 등록`, `출고 등록` 버튼을 상단에서 하단의 떠 있는 액션 그룹으로 옮겨 내역을 먼저 보고 등록할 수 있게 했습니다.
- 준비 품목 편집 시트에서 긴 품목명이 잘리지 않도록 품목명/현재고 영역과 수량 조절 영역을 위아래로 분리하고, 품목명은 자연스럽게 줄바꿈되도록 수정했습니다.
- 발주/배송 상단 필터는 `승인대기`, `진행중`, `완료` 3개 탭만 크게 두고 각 탭 옆에 숫자를 표시하며, `반려 1건`은 승인대기 아래 보조 항목으로 분리했습니다.
- 발주/배송 카드 액션 버튼도 `송장 등록`, `입고 확인`, `송장 상세` 같은 라벨이 한 줄로 유지되도록 버튼 폭/줄바꿈/아이콘 축소 규칙을 정리했습니다.
- 관리 > 수술 준비에서 수술 일정 삭제 기능을 추가했습니다. 삭제 시 확인창을 띄우고, 관련 수술 알림과 리마인더 상태도 함께 정리합니다.
- 예정 수술이 없을 때 빈 상태 문구를 추가했습니다.
- 표적 Playwright 검증으로 닫기 버튼 노출, 발주/배송 버튼 한 줄 유지, 수술 일정 등록/삭제를 확인했고, 전체 21개 브라우저 흐름도 다시 통과했습니다.

## PM 병렬 진행 결과 (바코드 제외)

로드맵에서 바코드 연동은 제외하고, 지금 바로 제품 완성도에 영향을 주는 세 축을 Codex 팀원들과 병렬로 나눠 진행했습니다.

- 배송 상세는 `shipping_events` 기반 데이터 구조로 바꿨습니다. 송장 등록 시 `배송중`/`주문접수` 이벤트가 저장되고, 입고 확인 시 `입고완료` 이벤트가 누적됩니다. 기존 `shipping_timeline` 데이터도 읽을 수 있게 호환 처리를 남겼습니다.
- 도매 설정은 단순 안내 화면에서 실제 설정 화면으로 확장했습니다. 거래처별 연결 여부, ID/비밀번호, 거래처별 자동발주 토글, 선호 거래처, 최대 주문금액 검증, 저장 전 변경 안내를 추가했습니다.
- 홈/재고/입출고/상태 카드의 긴 텍스트는 한 줄 말줄임 대신 두 줄까지 보여주고, 수량/시간/버튼 영역은 줄어들지 않게 고정해 모바일에서 정보가 갑자기 잘리는 느낌을 줄였습니다.
- 입출고 등록 액션은 긴 리스트 끝까지 내려가지 않아도 누를 수 있도록 하단 내비 바로 위에 떠 있는 액션 그룹으로 바꿨습니다. 품목 선택 리스트와 입출고 상세 시트 모두 X 닫기 버튼을 명확히 노출합니다.
- 배송 타임라인 문구는 `송장등록/발주승인`처럼 내부 처리 느낌이 강한 표현을 `배송중/주문접수`로 정리해 사용자 화면 기준으로 자연스럽게 맞췄습니다.
- 검증은 `npm run lint`, `npm test`, `npm run build`, 최신 UX E2E 7개, 최종 회귀 E2E 21개를 모두 통과했습니다.

## 하단/탭 UX 미세 조정

- 입출고 하단 액션 그룹은 기존 넓은 버튼 방향을 유지하되, 하단 내비게이션과 붙어 보이지 않도록 위쪽 여백을 더 확보했습니다.
- 입출고 날짜 헤더 오른쪽의 `입고 +n`, `출고 -n` 집계는 리스트 노이즈를 줄이기 위해 제거했습니다.
- 관리 화면의 가로 서브탭은 기존 구조를 유지하되, 오른쪽 끝에 얇은 페이드와 chevron 힌트를 추가해 더 넘길 수 있다는 느낌을 주었습니다.
- `npm run lint`, `npm test`, `npm run build`, 최신 UX E2E 7개, 최종 회귀 E2E 21개를 다시 통과했습니다.

## 데이터 레이어 분리

실제 백엔드/Firebase 전환을 준비하기 위해 저장소 접근 경계를 분리했습니다.

- `src/repositories/localRepository.js`를 추가해 현재 localStorage 어댑터를 repository 인터페이스 뒤로 감췄습니다.
- `src/repositories/appRepository.js`를 추가해 `users`, `items`, `txs`, `orders`, `surgeries`, `notifs`, `settings`, `session`, `authAttempts`를 도메인 단위 repository로 묶었습니다.
- `itemsApi`, `ordersApi`, `settingsApi`, `authApi`, `seed` 등 기존 API 모듈은 더 이상 storage를 직접 호출하지 않고 `appRepository`만 사용하게 정리했습니다.
- 초기 데이터 시드와 리셋도 repository를 통해 실행되므로, 나중에 Firestore/REST 구현체로 바꿀 때 앱 화면과 훅을 덜 건드릴 수 있습니다.
- repository 테스트를 추가해 collection/value repository의 `list/save/get/set/remove` 계약과 초기 데이터 복구 동작을 고정했습니다.
- `npm run lint`, `npm test` 14개 파일 132개 테스트, `npm run build`, 최종 회귀 E2E 21개를 모두 통과했습니다.

## 백엔드 스키마/보안 기준

데이터 레이어 다음 단계로 `clinicId` 기반 백엔드 전환 초안과 보안 기준을 정리했습니다.

- `docs/DATA_BACKEND_SCHEMA.md`에 Firestore/백엔드 collection 구조를 정의했습니다: `clinics/{clinicId}/users`, `items`, `txs`, `orders`, `surgeries`, `notifs`, `settings/app`, `vendorCredentials`, `auditLogs`.
- 모든 업무 데이터는 `clinicId` 하위에 격리하고, 요청자의 `auth.token.clinicId`와 문서의 `clinicId`가 일치해야 한다는 규칙을 명시했습니다.
- 도매몰 ID/PW는 `settings` 문서가 아니라 server-only `vendorCredentials`에 저장하고, AES-256-GCM + KMS/Secret Manager 키 분리를 전제로 했습니다.
- `docs/SECURITY_CHECKLIST.md`에 Critical/High/Medium 보안 체크리스트를 추가했습니다.
- `src/security/dataClassification.js`에 데이터 분류와 server-only 필드 목록을 코드로 추가했고, `securityPolicy.test.js`로 도매몰 비밀번호/세션성 데이터가 클라이언트 저장 가능 필드로 통과하지 않도록 기준을 고정했습니다.
- `npm run lint`, `npm test` 15개 파일 134개 테스트, `npm run build`, 최종 회귀 E2E 21개를 모두 통과했습니다.

## 클리닉 네임스페이스/도매 계정 분리

보안 기준 문서에 맞춰 현재 로컬 앱에서도 백엔드 전환 구조를 한 단계 더 반영했습니다.

- local repository 저장 키를 `clinics/demo-clinic/...` 형태로 감싸, 재고/입출고/발주/수술/설정/도매 계정이 병원 단위 네임스페이스 아래에 저장되도록 했습니다.
- 세션과 로그인 실패 횟수는 앱 전역성 데이터로 남기고, 업무 데이터는 활성 `clinicId`를 바꾸면 분리되어 읽히는 테스트를 추가했습니다.
- 도매 설정의 `vendors[]`에서는 `username/password`를 제거했습니다. 화면 입력은 유지하되 `vendorCredentials` repository/API로 분리해 저장합니다.
- 기존 설정에 섞여 있던 도매몰 ID/PW는 로드 시 `vendorCredentials`로 이관하고, 이후 `settings` 저장 시에는 계정 필드를 다시 쓰지 않도록 막았습니다.
- server-only 필드 정책에 `vendors.username`과 `vendorCredentials.username*` 암호화 필드를 추가해 ID도 비밀번호와 같은 민감 정보로 다루게 했습니다.
- `npm run lint`, `npm test` 15개 파일 136개 테스트, `npm run build`, 최신 UX E2E 7개, 최종 회귀 E2E 21개를 모두 통과했습니다.

## 감사 로그 추가

운영 중 “누가 언제 무엇을 바꿨는지” 추적할 수 있도록 audit log 기반을 추가했습니다.

- `auditLogs` repository/API를 추가하고, 로컬에서도 clinic namespace 아래에 저장되도록 연결했습니다.
- 입출고 등록, 발주 요청/승인/반려, 송장 등록, 입고 확인, 수술 등록/준비확인/품목수정/삭제가 audit log 이벤트를 남기도록 했습니다.
- 감사 로그에는 actor, action, entity, metadata, created_at을 남기되 `username/password/token/session/pin/card` 계열 metadata 키는 저장 전에 `[redacted]`로 마스킹합니다.
- 수술 로그에는 환자명을 직접 남기지 않고, 환자 정보 존재 여부만 기록하도록 했습니다.
- audit log API 테스트와 보안 정책 테스트를 추가했습니다.
- `npm run lint`, `npm test` 16개 파일 138개 테스트, `npm run build`, 최신 UX E2E 7개, 최종 회귀 E2E 21개를 모두 통과했습니다.

## 활동 로그/배송완료 알림

감사 로그를 실제 운영 화면에서 확인할 수 있게 만들고, 송장 등록 후 배송 상태가 배달완료 알림으로 이어지는 흐름을 보강했습니다.

- 관리 화면에 `활동 로그` 탭을 추가했습니다. 전체/입출고/발주/수술/보안 필터로 audit log를 확인할 수 있습니다.
- 로그 행에는 처리자, 액션, 대상 ID, 수량 변화, 배송 상태, 송장 끝자리, 메모 등을 표시합니다.
- 송장 등록된 진행중 주문에 `배송 갱신` 액션을 추가했습니다. 데모 환경에서는 `배송출발` 다음 `배달완료` 순서로 상태가 누적됩니다.
- `배달완료` 상태가 되면 `배달완료` 알림을 생성하고, 배송 상세에 `입고 확인이 필요합니다` 안내를 표시합니다.
- audit log 마스킹 규칙이 `shipping_status`를 오탐해 `[redacted]`로 표시하던 문제를 고쳤습니다.
- 배송 갱신/배달완료 단위 테스트를 추가했습니다.
- `npm run lint`, `npm test` 16개 파일 140개 테스트, `npm run build`, 최신 UX E2E 7개, 최종 회귀 E2E 21개를 모두 통과했습니다.

## 백엔드 골격/프론트 어댑터

자동화 서버를 붙일 수 있도록 백엔드 기본 구조와 프론트 API 모드 경계를 추가했습니다.

- `server/` 폴더에 Node HTTP 기반 API skeleton을 추가했습니다. 현재는 의존성 설치 없이 실행/테스트 가능하도록 만들었고, 라우트/서비스 경계는 Express/Fastify로 교체하기 쉽게 분리했습니다.
- 기본 라우트는 `GET /health`, `GET /api/orders`, `POST /api/orders/:orderId/approve`, `POST /api/tracking/refresh`, `GET/POST /api/vendor-credentials/:vendorId`입니다.
- 보안 기본값으로 no-store/security headers, CORS allow-list, clinicId 헤더 컨텍스트, 간단한 rate limit, vendor credential write 기본 비활성화를 넣었습니다.
- `server/services/trackingService.js`와 `vendorCredentialService.js`를 추가해 실제 스윗트래커/암호화 저장소로 교체할 인터페이스를 마련했습니다.
- `.env.example`에 서버 포트/호스트/CORS/rate limit/internal token과 프론트 `VITE_DENTALSTOCK_API_MODE` 값을 정리했습니다.
- 프론트에는 `apiMode`, `repositoryAdapter`, `remoteRepository`를 추가했습니다. 현재 앱은 안정성을 위해 계속 local storage를 active storage로 쓰고, `server` 모드는 기능 플래그로 노출만 합니다.
- 서버 테스트는 포트를 열지 않고 handler를 직접 호출해 sandbox/CI에서도 안정적으로 돌도록 구성했습니다.
- `npm run server:check`, `npm run lint`, `npm test` 18개 파일 146개 테스트, `npm run build`, 최신 UX E2E 7개, 최종 회귀 E2E 21개를 모두 통과했습니다.
