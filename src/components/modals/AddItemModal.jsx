...
export function AddItemModal({setItems, onClose, showToast}) {
  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h2 style={{margin:0,fontSize:26,fontWeight:700,color:T.grey900}}>품목 추가</h2><button onClick={onClose} style={{border:"none",background:"none",cursor:"pointer"}}><X size={24} color={T.grey500}/></button></div>
      <p style={{margin:"0 0 6px",fontSize:19,fontWeight:600,color:T.grey700}}>품목명</p><Inp value={name} onChange={e=>setName(e.target.value)} placeholder="예: 라텍스 장갑 (L)" style={{marginBottom:12}}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        <div><p style={{margin:"0 0 6px",fontSize:19,fontWeight:600,color:T.grey700}}>단위</p><Inp value={unit} onChange={e=>setUnit(e.target.value)} placeholder="개/박스"/></div>
      ...
      <p style={{margin:"0 0 6px",fontSize:19,fontWeight:600,color:T.grey700}}>보관 위치</p><Inp value={loc} onChange={e=>setLoc(e.target.value)} placeholder="예: 창고 A-1" style={{marginBottom:16}}/>
      <button onClick={submit} style={{width:"100%",padding:"16px 0",borderRadius:9999,border:"none",background:T.blue500,color:T.white,fontSize:19,fontWeight:600,cursor:"pointer",fontFamily:font}}>추가 완료</button>
    </div>
  );
}
