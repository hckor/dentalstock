...
export function AddItemModal({setItems, onClose, showToast}) {
  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      ...
      <p style={{margin:"0 0 6px",fontSize:14,fontWeight:600,color:T.grey700}}>품목명</p><Inp value={name} onChange={e=>setName(e.target.value)} placeholder="예: 라텍스 장갑 (L)" style={{marginBottom:12}}/>
      <p style={{margin:"0 0 8px",fontSize:14,fontWeight:600,color:T.grey700}}>카테고리</p>
      <div style={{display:"flex",gap:8,marginBottom:12}}>{CATEGORIES.map(c=><button key={c.id} onClick={()=>setCatId(c.id)} style={{flex:1,padding:"16px 0",borderRadius:9999,border:"none",cursor:"pointer",fontFamily:font,fontSize:12,fontWeight:600,background:catId===c.id?c.color:T.grey100,color:catId===c.id?T.white:T.grey700}}>{c.name}</button>)}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        <div><p style={{margin:"0 0 6px",fontSize:14,fontWeight:600,color:T.grey700}}>단위</p><Inp value={unit} onChange={e=>setUnit(e.target.value)} placeholder="개/박스"/></div>
        <div><p style={{margin:"0 0 6px",fontSize:14,fontWeight:600,color:T.grey700}}>최소수량</p><Inp value={minQty} onChange={e=>setMinQty(parseInt(e.target.value)||1)} type="number"/></div>
      </div>
      <p style={{margin:"0 0 6px",fontSize:14,fontWeight:600,color:T.grey700}}>보관 위치</p><Inp value={loc} onChange={e=>setLoc(e.target.value)} placeholder="예: 창고 A-1" style={{marginBottom:16}}/>
