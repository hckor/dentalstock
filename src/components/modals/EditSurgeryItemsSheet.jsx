...
export function EditSurgeryItemsSheet({initialItems, allItems, onSave, onClose, title}) {
  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      ...
      {title && <p style={{margin:"0 0 14px", fontSize:14, color:T.grey500}}>{title}</p>}

      {!picking && (
        <>
          <div style={{background:T.grey50, borderRadius:12, padding:"12px 8px", marginBottom:12}}>
            {draft.length===0 ? (
              <p style={{margin:0, padding:"16px 16px", fontSize:14, color:T.grey500, textAlign:"center"}}>품목이 비어 있어요. 아래에서 추가하세요.</p>
            ) : draft.map((r,i)=>{
              const item = allItems.find(it=>it.id===r.item_id);
              const enough = item && item.current_qty>=r.qty;
              return (
                <div key={r.item_id} style={{display:"flex", alignItems:"center", gap:10, padding:"16px 16px", borderTop:i===0?"none":`1px solid ${T.grey100}`}}>
                  ...
                  <div style={{display:"flex", alignItems:"center", gap:6}}>
                    <button onClick={()=>updateQty(r.item_id, r.qty-1)} style={{width:28, height:28, borderRadius:9999, border:`1px solid ${T.grey200}`, background:T.white, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}><Minus size={16} color={T.grey700}/></button>
                    ...
                    <button onClick={()=>updateQty(r.item_id, r.qty+1)} style={{width:28, height:28, borderRadius:9999, border:"none", background:T.blue500, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}><Plus size={16} color={T.white}/></button>
                  </div>
          ...
          <button onClick={()=>setPicking(true)} disabled={candidates.length===0}
            style={{width:"100%", padding:"16px 0", borderRadius:12, border:`1.5px dashed ${T.grey300}`, background:T.white, cursor:candidates.length===0?"default":"pointer", fontFamily:font, fontSize:16, fontWeight:600, color:candidates.length===0?T.grey400:T.blue500, marginBottom:16, display:"flex", alignItems:"center", justifyContent:"center", gap:6}}>
            <Plus size={16}/> 품목 추가
          </button>
          <div style={{display:"flex", gap:10, paddingBottom:8}}>
            <button onClick={onClose} style={{flex:1, padding:"16px 0", borderRadius:9999, border:`1px solid ${T.grey200}`, background:T.white, cursor:"pointer", fontFamily:font, fontSize:16, fontWeight:600, color:T.grey700}}>취소</button>
            <button onClick={()=>{onSave(draft); onClose();}} style={{flex:2, padding:"16px 0", borderRadius:9999, border:"none", background:T.blue500, cursor:"pointer", fontFamily:font, fontSize:16, fontWeight:600, color:T.white}}>저장</button>
          </div>
        </>
      )}
      ...
      {picking && (
        <>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8}}>
            <p style={{margin:0, fontSize:14, fontWeight:600, color:T.grey700}}>추가할 품목 선택</p>
            <button onClick={()=>setPicking(false)} style={{border:"none", background:"none", cursor:"pointer", fontSize:14, color:T.grey500, fontFamily:font}}>← 뒤로</button>
          </div>
          {candidates.length===0 ? (
            <p style={{margin:0, padding:"24px 0", fontSize:14, color:T.grey500, textAlign:"center"}}>모든 품목이 이미 추가되어 있어요.</p>
          ) : candidates.map((item,i)=>(
            <div key={item.id}>
              <button onClick={()=>addRow(item.id)} style={{width:"100%", display:"flex", alignItems:"center", gap:12, padding:"16px 0", background:"none", border:"none", cursor:"pointer", fontFamily:font}}>
