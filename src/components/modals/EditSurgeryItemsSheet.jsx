      ...
      {!picking && (
        <>
          <div style={{background:T.grey50, borderRadius:12, padding:"8px 4px", marginBottom:12}}>
            {draft.length===0 ? (
              <p style={{margin:0, padding:"18px 12px", fontSize: 16, color:T.grey500, textAlign:"center"}}>품목이 비어 있어요. 아래에서 추가하세요.</p>
            ) : draft.map((r,i)=>{
      ...
      {picking && (
        <>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8}}>
            <p style={{margin:0, fontSize: 16, fontWeight:600, color:T.grey700}}>추가할 품목 선택</p>
            <button onClick={()=>setPicking(false)} style={{border:"none", background:"none", cursor:"pointer", fontSize: 16, color:T.grey500, fontFamily:font}}>← 뒤로</button>
          </div>
          {candidates.length===0 ? (
            <p style={{margin:0, padding:"24px 0", fontSize: 16, color:T.grey500, textAlign:"center"}}>모든 품목이 이미 추가되어 있어요.</p>
