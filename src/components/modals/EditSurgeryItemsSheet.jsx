  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6}}>
        <h2 style={{margin:0, fontSize:26, fontWeight:700, color:T.grey900}}>준비 품목 편집</h2>
        <button onClick={onClose} style={{border:"none", background:"none", cursor:"pointer"}}><X size={24} color={T.grey500}/></button>
      </div>
      {title && <p style={{margin:"0 0 14px", fontSize:19, color:T.grey500}}>{title}</p>}

      {!picking && (
        <>
          <div style={{background:T.grey50, borderRadius:12, padding:"8px 4px", marginBottom:12}}>
            {draft.length===0 ? (
              <p style={{margin:0, padding:"18px 12px", fontSize:19, color:T.grey500, textAlign:"center"}}>품목이 비어 있어요. 아래에서 추가하세요.</p>
            ) : draft.map((r,i)=>{
              ...
              return (
                <div key={r.item_id} style={{display:"flex", alignItems:"center", gap:10, padding:"14px 16px", borderTop:i===0?"none":`1px solid ${T.grey100}`}}>
                  <div style={{flex:1, minWidth:0}}>
                    <p style={{margin:0, fontSize:20, fontWeight:600, color:T.grey900, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{item?.name||"삭제된 품목"}</p>
                  ...
                  <div style={{display:"flex", alignItems:"center", gap:6}}>
                    ...
                    <input value={r.qty} onChange={e=>updateQty(r.item_id, e.target.value)} type="number" min={1}
                      style={{width:44, textAlign:"center", padding:"6px 0", borderRadius:8, border:`1px solid ${T.grey200}`, background:T.white, fontSize:20, fontWeight:700, color:T.grey900, fontFamily:font, outline:"none"}}/>
          ...
          <button onClick={()=>setPicking(true)} disabled={candidates.length===0}
            style={{width:"100%", padding:"16px 0", borderRadius:12, border:`1.5px dashed ${T.grey300}`, background:T.white, cursor:candidates.length===0?"default":"pointer", fontFamily:font, fontSize:20, fontWeight:600, color:candidates.length===0?T.grey400:T.blue500, marginBottom:16, display:"flex", alignItems:"center", justifyContent:"center", gap:6}}>
            <Plus size={20}/> 품목 추가
          </button>
          <div style={{display:"flex", gap:10, paddingBottom:8}}>
            <button onClick={onClose} style={{flex:1, padding:"18px 0", borderRadius:9999, border:`1px solid ${T.grey200}`, background:T.white, cursor:"pointer", fontFamily:font, fontSize:22, fontWeight:600, color:T.grey700}}>취소</button>
            <button onClick={()=>{onSave(draft); onClose();}} style={{flex:2, padding:"18px 0", borderRadius:9999, border:"none", background:T.blue500, cursor:"pointer", fontFamily:font, fontSize:22, fontWeight:600, color:T.white}}>저장</button>
          </div>
        </>
      )}
      ...
      {picking && (
        <>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8}}>
            <p style={{margin:0, fontSize:19, fontWeight:600, color:T.grey700}}>추가할 품목 선택</p>
            <button onClick={()=>setPicking(false)} style={{border:"none", background:"none", cursor:"pointer", fontSize:19, color:T.grey500, fontFamily:font}}>← 뒤로</button>
          </div>
          {candidates.length===0 ? (
            <p style={{margin:0, padding:"24px 0", fontSize:19, color:T.grey500, textAlign:"center"}}>모든 품목이 이미 추가되어 있어요.</p>
            ...
            <div key={item.id}>
              <button onClick={()=>addRow(item.id)} style={{width:"100%", display:"flex", alignItems:"center", gap:12, padding:"18px 0", background:"none", border:"none", cursor:"pointer", fontFamily:font}}>
                ...
                <div style={{flex:1, textAlign:"left"}}>
                  ...
                  <p style={{margin:0, fontSize:18, color:T.grey500}}>현재 {item.current_qty}{item.unit}</p>
                </div>
