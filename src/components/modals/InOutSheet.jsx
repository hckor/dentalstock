...
export function InOutSheet({modal, selItem, form, setForm, onCommit, onClose}) {
  ...
  const accentSoft = isOut ? T.red50 : T.blue50;
  const accentBorder = isOut ? T.red500+"33" : T.blue500+"33";
  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16}}>
        <h2 style={{margin:0, fontSize: 24, fontWeight:700, color:accent}}>{isOut?"출고 등록":"입고 등록"}</h2>
      ...
      <div style={{display:"flex", alignItems:"center", gap:16, marginBottom:12}}>
        ...
        <button onClick={()=>setForm(f=>({...f,qty:f.qty+1}))} style={{width:44,height:44,borderRadius:9999,border:"none",background:accent,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Plus size={22} color={T.white}/></button>
      </div>
      ...
      <button
        onClick={onCommit}
        disabled={overStock}
        style={{width:"100%",padding:"16px 0",borderRadius:9999,border:"none",cursor:overStock?"not-allowed":"pointer",fontFamily:font,fontSize: 16,fontWeight:600,color:T.white,background:overStock?T.grey300:accent}}
      >
