...
export function InOutSheet({modal, selItem, form, setForm, onCommit, onClose}) {
  return (
    <div style={{padding:"16px 20px 0"}}>
      ...
      <div style={{background:T.grey50, borderRadius:12, padding:"12px 14px", marginBottom:20, border:`1.5px solid ${modal==="in"?T.blue500+"33":T.red500+"33"}`}}>
        <p style={{margin:0, fontSize:14, fontWeight:600, color:T.grey900}}>{selItem.name}</p>
        <p style={{margin:"4px 0 0", fontSize:13, color:T.grey600}}>현재 재고 <span style={{fontWeight:700, color:modal==="in"?T.blue500:T.red500}}>{selItem.current_qty}{selItem.unit}</span></p>
      </div>
      ...
      <div style={{display:"flex", gap:6, marginBottom:20}}>
        {[10, 50, 100].map(n => (
          <button key={n} onClick={()=>setForm(f=>({...f,qty:f.qty+n}))} style={{flex:1, padding:"8px 0", borderRadius:9999, border:`1px solid #bfdbfe`, background:T.blue50, color:T.blue500, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:font}}>
            +{n}
          </button>
        ))}
      </div>
