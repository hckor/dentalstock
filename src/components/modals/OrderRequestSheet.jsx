...
export function OrderRequestSheet({item, currentUser, onSubmit, onClose}) {
  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      ...
      <div style={{background:T.teal50, borderRadius:12, padding:"16px 16px", marginBottom:20, border:`1px solid ${T.teal500}22`}}>
        ...
        <p style={{margin:0, fontSize:14, color:T.grey600}}>
      ...
      <p style={{margin:"0 0 8px", fontSize:14, fontWeight:600, color:T.grey700}}>요청 수량</p>
      <div style={{display:"flex", alignItems:"center", gap:16, marginBottom:20}}>
        <button onClick={()=>setQty(q=>Math.max(1,q-1))} style={{width:56, height:56, borderRadius:9999, border:`1.5px solid ${T.grey200}`, background:T.white, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}><Minus size={20} color={T.grey700}/></button>
        ...
        <button onClick={()=>setQty(q=>q+1)} style={{width:56, height:56, borderRadius:9999, border:"none", background:T.blue500, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}><Plus size={20} color={T.white}/></button>
      </div>
      <p style={{margin:"0 0 8px", fontSize:14, fontWeight:600, color:T.grey700}}>메모 <span style={{fontWeight:400, color:T.grey400}}>(선택)</span></p>
      <Inp value={note} onChange={e=>setNote(e.target.value)} placeholder="예: 재고 급하게 필요합니다" style={{marginBottom:16}}/>
      <div style={{display:"flex", alignItems:"center", gap:8, padding:"16px 16px", background:T.grey50, borderRadius:12, marginBottom:20, border:`1px solid ${T.grey200}`}}>
        <Avatar name={currentUser.name} role={currentUser.role} size={28}/>
        <p style={{margin:0, fontSize:14, color:T.grey600}}><span style={{fontWeight:600, color:T.grey900}}>{currentUser.name}</span> · 장바구니 확인 후 일괄 발주 요청</p>
      </div>
