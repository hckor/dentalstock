...
export function ReceiptConfirmSheet({item, orders, onConfirm, onClose}) {
  ...
  return (
    <div style={{padding:"16px 20px 0"}}>
      ...
      {/* 발주 정보 */}
      <div style={{background:T.teal50, borderRadius:12, padding:"16px 16px", marginBottom:20, border:`1px solid ${T.teal500}33`}}>
        <p style={{margin:0, fontSize:14, color:T.grey600, marginBottom:4}}>발주 정보</p>
      ...
      <p style={{margin:"0 0 6px", fontSize:14, fontWeight:600, color:T.grey700}}>실제 수령한 수량</p>
      <p style={{margin:"0 0 10px", fontSize:12, color:T.grey500}}>실제로 받은 수량을 확인 후 입력해주세요</p>
      <div style={{display:"flex", alignItems:"center", gap:16, marginBottom:20}}>
        <button onClick={()=>setQty(q=>Math.max(1,q-1))} style={{width:56, height:56, borderRadius:9999, border:`1.5px solid ${T.grey200}`, background:T.white, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}>
          <Minus size={20} color={T.grey700}/>
        </button>
        ...
        <button onClick={()=>setQty(q=>q+1)} style={{width:56, height:56, borderRadius:9999, border:"none", background:T.blue500, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}>
          <Plus size={20} color={T.white}/>
        </button>
      </div>
      <p style={{margin:"0 0 8px", fontSize:14, fontWeight:600, color:T.grey700}}>특이사항 (선택)</p>
      <Inp value={note} onChange={e=>setNote(e.target.value)} placeholder="예: 1박스 파손, 일부 수령" style={{marginBottom:20}}/>
      <div style={{background:T.grey50, borderRadius:10, padding:"16px 16px", marginBottom:20, display:"flex", alignItems:"center", gap:8}}>
        <CircleDot size={16} color={T.green500}/>
