        ...
        <div style={{display:"flex", gap:6, overflowX:"auto", scrollbarWidth:"none"}}>
          {tabs.map(t => (
            <button key={t.id} onClick={()=>setAdminTab(t.id)}
              style={{flexShrink:0, padding:"8px 16px", border:"none", borderRadius:9999, cursor:"pointer", fontFamily:font, fontSize:13, fontWeight:600,
        ...
        {adminTab === "orders" && (
          <>
            {pendingOrders.length > 0 ? (
              <>
                <p style={{margin:"0 0 12px", fontSize:13, fontWeight:600, color:T.grey600}}>승인 대기 {pendingOrders.length}</p>
                      ...
                      <Card key={order.id} style={{padding:"16px"}}>
                        ...
                        <div style={{display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:12}}>
                          ...
                          <p style={{margin:0, fontSize:18, fontWeight:700, color:T.grey900, flexShrink:0, fontVariantNumeric:"tabular-nums"}}>
                            {order.qty}<span style={{fontSize:13, fontWeight:400, color:T.grey500}}>{item?.unit}</span>
                          </p>
                        </div>

                        {/* 재고 현황 */}
                        <div style={{background:T.grey50, borderRadius:10, padding:"9px 12px", marginBottom:10, display:"flex", alignItems:"center", gap:8}}>
                        ...
                        {/* 요청 메모 */}
                        {order.note && (
                          <div style={{background:T.orange50, borderRadius:10, padding:"9px 12px", marginBottom:12}}>
                            <p style={{margin:"0 0 2px", fontSize:11, fontWeight:600, color:T.orange500}}>요청 메모</p>
                            <p style={{margin:0, fontSize:13, color:T.grey700}}>{order.note}</p>
                          </div>
                        )}
                        ...
                        <div style={{display:"grid", gridTemplateColumns:"1fr 2fr", gap:8}}>
                          <button onClick={()=>{setReviewModal({order, action:"rejected"}); setReviewNote("");}}
                            style={{padding:"12px 0", borderRadius:9999, border:`1.5px solid ${T.grey200}`, background:T.white, color:T.red500, fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:font}}>
                            거절
                          </button>
                          <button onClick={()=>{setReviewModal({order, action:"approved"}); setReviewNote("");}}
                            style={{padding:"12px 0", borderRadius:9999, border:"none", background:T.blue500, color:T.white, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:font}}>
                            발주 승인
                          </button>
                        </div>
                      </Card>
              ...
              <div style={{textAlign:"center", padding:"40px 20px"}}>
                <p style={{margin:0, fontSize:15, fontWeight:600, color:T.grey800}}>검토할 발주 요청이 없어요</p>
            ...
            {doneOrders.length > 0 && (
              <>
                <p style={{margin:"0 0 10px", fontSize:13, fontWeight:600, color:T.grey600}}>처리 이력</p>
                <Card>
                  {doneOrders.map((o, i) => {
                    ...
                    return (
                      <div key={o.id}>
                        <div style={{display:"flex", alignItems:"center", gap:12, padding:"12px 16px"}}>
                          <div style={{width:34, height:34, borderRadius:10, background:os.bg, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0}}>
                            {o.status==="ordered"  ? <Truck size={15} color={os.text}/> :
                             o.status==="received" ? <PackageCheck size={15} color={os.text}/> :
                                                     <XCircle size={15} color={os.text}/>}
                          </div>
                          <div style={{flex:1, minWidth:0}}>
        ...
        {adminTab === "staff" && (
          <>
            ...
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:16}}>
              {[
                ...
                <Card key={s.label} style={{padding:"12px 10px"}}>
                  ...
                  <p style={{margin:0, fontSize:22, fontWeight:700, color:s.color, fontVariantNumeric:"tabular-nums"}}>{s.value}</p>
                </Card>
              ))}
            </div>

            <p style={{margin:"0 0 10px", fontSize:13, fontWeight:600, color:T.grey600}}>직원 목록</p>
                    ...
                    <div style={{display:"flex", alignItems:"center", gap:12, padding:"14px 16px", opacity:u.active?1:0.45}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{display:"flex", alignItems:"center", gap:6}}>
                          <p style={{margin:0, fontSize:14, fontWeight:600, color:T.grey900}}>{u.name}</p>
                      ...
                      {!isMe && (
                        <button onClick={()=>setUsers(p=>p.map(x=>x.id===u.id?{...x,active:!x.active}:x))}
                          style={{padding:"6px 14px", borderRadius:9999, border:"none", cursor:"pointer", fontFamily:font, fontSize:12, fontWeight:600, background:u.active?T.red50:T.green50, color:u.active?T.red500:T.green500}}>
                          {u.active?"비활성":"활성화"}
                        </button>
                      )}
                    </div>
            ...
            <button onClick={onLogout}
              style={{width:"100%", padding:"14px 0", borderRadius:9999, border:`1.5px solid ${T.grey200}`, background:T.white, color:T.grey700, fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:font, display:"flex", alignItems:"center", justifyContent:"center", gap:8}}>
      ...
      {reviewModal && (
        <div style={{position:"absolute", inset:0, background:"rgba(2,9,19,0.5)", zIndex:99, display:"flex", justifyContent:"center", alignItems:"flex-end"}} onClick={()=>setReviewModal(null)}>
          <div style={{background:T.white, borderRadius:"16px 16px 0 0", width:"100%", padding:"20px 20px 40px"}} onClick={e=>e.stopPropagation()}>
            ...
            <p style={{margin:"0 0 16px", fontSize:14, color:T.grey600}}>
              {reviewModal.action==="approved"
            ...
            {(() => {
              ...
              return (
                <div style={{background:T.grey50, borderRadius:12, padding:"12px 14px", marginBottom:16, border:`1px solid ${T.grey200}`}}>
                  ...
                  <p style={{margin:"4px 0 0", fontSize:13, color:T.grey600}}>
                    {reviewModal.order.requested_by} · <span style={{fontWeight:700, color:T.teal500}}>{reviewModal.order.qty}{item?.unit}</span>
                  </p>
                </div>
              );
            })()}
            <p style={{margin:"0 0 8px", fontSize:13, fontWeight:600, color:T.grey700}}>
            ...
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}>
              <button onClick={()=>setReviewModal(null)}
                style={{padding:"14px 0", borderRadius:9999, border:`1.5px solid ${T.grey200}`, background:T.white, color:T.grey700, fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:font}}>
                취소
              </button>
              <button onClick={confirmReview}
                style={{padding:"14px 0", borderRadius:9999, border:"none", background:reviewModal.action==="approved"?T.blue500:T.red500, color:T.white, fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:font}}>
