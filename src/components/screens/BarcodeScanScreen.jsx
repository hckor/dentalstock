    ...
    <div style={{position:"fixed", inset:0, zIndex:200, display:"flex", flexDirection:"column", background:"#000"}}>
      ...
      <div style={{flex:1, position:"relative", overflow:"hidden"}}>
        ...
        {!camReady && !camError && (
          <div style={{position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", background:"#0a0e14"}}>
            <div style={{textAlign:"center"}}>
              ...
              <p style={{color:"rgba(255,255,255,0.7)", fontSize:19, margin:0}}>카메라 연결 중...</p>
            </div>
          </div>
        )}
        {camError && (
          <div style={{position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", background:"#0a0e14"}}>
            <div style={{textAlign:"center", padding:"0 32px"}}>
              <p style={{color:"#f87171", fontSize:22, fontWeight:600, margin:"0 0 6px"}}>{camError}</p>
              <p style={{color:"rgba(255,255,255,0.5)", fontSize:19, margin:0}}>아래 검색을 이용해주세요</p>
            </div>
          </div>
        )}
        ...
        <div style={{position:"absolute", top:0, left:0, right:0, padding:"54px 16px 16px", display:"flex", alignItems:"center", justifyContent:"space-between"}}>
          ...
          <p style={{margin:0, fontSize:19, fontWeight:700, color:"#fff"}}>바코드 스캔</p>
          <div style={{display:"flex", gap:8}}>
            ...
            <button onClick={()=>setShowInput(v=>!v)} style={{height:36, padding:"0 12px", borderRadius:9999, border:"none", background:showInput?"#2563eb":"rgba(0,0,0,0.4)", cursor:"pointer", fontFamily:font, fontSize:18, fontWeight:600, color:"#fff", display:"flex", alignItems:"center", gap:5}}>
        ...
        {/* 스캔된 결과 표시 */}
        {scanResult && !selected && (
          <div style={{position:"absolute", bottom:170, left:16, right:16, background:"rgba(0,0,0,0.75)", borderRadius:12, padding:"14px 18px", textAlign:"center"}}>
            <p style={{margin:0, fontSize:18, color:"rgba(255,255,255,0.6)"}}>스캔됨</p>
            <p style={{margin:"2px 0 0", fontSize:20, fontWeight:700, color:"#fff"}}>{scanResult}</p>
            <p style={{margin:"2px 0 0", fontSize:18, color:"#f87171"}}>품목을 찾을 수 없어요 — 아래에서 검색해주세요</p>
          </div>
        )}
        ...
        {camReady && !selected && !scanResult && (
          <div style={{position:"absolute", bottom:170, left:0, right:0, textAlign:"center"}}>
            <p style={{margin:0, fontSize:19, color:"rgba(255,255,255,0.7)"}}>바코드를 사각 영역에 맞춰주세요</p>
            <p style={{margin:"4px 0 0", fontSize:16, color:"rgba(255,255,255,0.4)"}}>자동으로 인식됩니다</p>
          </div>
        )}
      </div>
      ...
      <div style={{background:T.white, borderRadius:"20px 20px 0 0", padding:"16px 16px 32px", minHeight:160, flexShrink:0}}>
        ...
        {(!selected) && (
          <div style={{position:"relative", marginBottom:12}}>
            <input
              ...
              autoFocus={showInput}
              style={{width:"100%", padding:"16px 18px", borderRadius:10, border:`1px solid ${T.grey200}`, background:T.grey50, fontSize:20, color:T.grey800, fontFamily:font, outline:"none", boxSizing:"border-box"}}
            />
          </div>
        )}
        ...
        {results.length > 0 && !selected && (
          <div style={{background:T.white, borderRadius:10, border:`1px solid ${T.grey200}`, overflow:"hidden", marginBottom:12}}>
            {results.map((item, i) => {
              ...
              return (
                <div key={item.id}>
                  <button onClick={()=>handleSelect(item)} style={{width:"100%", padding:"16px 18px", border:"none", background:"none", cursor:"pointer", fontFamily:font, display:"flex", alignItems:"center", gap:10, textAlign:"left"}}>
                    <div style={{flex:1, minWidth:0}}>
                      ...
                      <p style={{margin:"1px 0 0", fontSize:18, color:T.grey500}}>현재 {item.current_qty}{item.unit}</p>
                    </div>
                    <span style={{flexShrink:0, fontSize:16, fontWeight:600, color:sc.text, background:sc.bg, padding:"3px 8px", borderRadius:9999}}>{sc.label}</span>
                  </button>
        ...
        {selected && (
          <div>
            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14}}>
              <div>
                <p style={{margin:"0 0 2px", fontSize:16, color:T.green500, fontWeight:700}}>✓ 품목 확인</p>
                <p style={{margin:0, fontSize:26, fontWeight:700, color:T.grey900}}>{selected.name}</p>
                <p style={{margin:"2px 0 0", fontSize:18, color:T.grey500}}>현재 재고 <span style={{fontWeight:700, color:T.blue500}}>{selected.current_qty}{selected.unit}</span></p>
              </div>
            ...
            <div style={{display:"flex", alignItems:"center", gap:12, marginBottom:16}}>
              <p style={{margin:0, fontSize:19, color:T.grey600}}>수량</p>
              <div style={{flex:1}}/>
              ...
              <p style={{margin:0, fontSize:26, fontWeight:700, color:T.grey900, minWidth:36, textAlign:"center", fontVariantNumeric:"tabular-nums"}}>{qty}</p>
            ...
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
              <button onClick={()=>handleAction("in")} style={{padding:"18px 0", borderRadius:9999, border:"none", background:T.blue500, color:T.white, fontSize:20, fontWeight:700, cursor:"pointer", fontFamily:font}}>
                입고 +{qty}
              </button>
              <button onClick={()=>handleAction("out")} style={{padding:"18px 0", borderRadius:9999, border:"none", background:T.red500, color:T.white, fontSize:20, fontWeight:700, cursor:"pointer", fontFamily:font}}>
                출고 -{qty}
              </button>
            </div>
          </div>
        ...
        {!selected && results.length === 0 && !query && (
          <p style={{margin:0, fontSize:19, color:T.grey400, textAlign:"center", padding:"14px 0"}}>
