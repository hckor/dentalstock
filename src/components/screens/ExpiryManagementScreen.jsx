  ...
  return (
    <div style={{marginBottom:20}}>
      <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:8}}>
        ...
        <p style={{margin:0, fontSize: 16, fontWeight:700, color:T.grey700}}>{title}</p>
      </div>
      <Card>
        {items.map((item, i) => {
      ...
      <div style={{flex:1, overflowY:"auto", padding:"16px"}}>
        {expiryItems.length === 0 ? (
          <div style={{textAlign:"center", padding:"60px 0"}}>
            <p style={{margin:0, fontSize: 20, fontWeight:600, color:T.grey700}}>유통기한 정보가 없어요</p>
            <p style={{margin:"6px 0 0", fontSize: 16, color:T.grey500}}>품목 편집에서 유통기한을 등록해보세요</p>
          </div>
        ) : (
          <>
            {expired.length > 0 && (
              <div style={{marginBottom:20}}>
                <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:8}}>
                  ...
                  <p style={{margin:0, fontSize: 16, fontWeight:700, color:T.grey700}}>이미 만료됨 · 사용 금지</p>
                </div>
                  ...
                  {expired.map((item, i) => (
                    <div key={item.id}>
                      <div style={{display:"flex", alignItems:"center", gap:12, padding:"18px 20px"}}>
                        ...
                        <span style={{flexShrink:0, fontSize: 16, fontWeight:700, color:T.red500, marginRight:8}}>만료됨</span>
                        <button onClick={()=>handleOut(item)} style={{flexShrink:0, padding:"12px 16px", borderRadius:9999, border:"none", background:T.red500, color:T.white, fontSize: 16, fontWeight:600, cursor:"pointer", fontFamily:font, display:"flex", alignItems:"center", gap:4}}>
            ...
            <p style={{margin:"8px 0 0", fontSize: 16, color:T.grey400, lineHeight:1.6, textAlign:"center", paddingBottom:24}}>
