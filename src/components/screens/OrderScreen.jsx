  ...
  return (
    <div style={{padding:"16px"}}>
      ...
      <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:12}}>
        ...
        <p style={{margin:0, fontSize: 20, fontWeight:700, color:T.grey900}}>발주 장바구니</p>
        {cart.length > 0 && <span style={{fontSize: 16, fontWeight:700, color:T.blue500}}>({cart.length})</span>}
      </div>
      ...
      {cart.length === 0 ? (
        <button onClick={()=>{}} style={{width:"100%", padding:"20px 16px", borderRadius:14, border:`1.5px dashed ${T.grey200}`, background:"none", cursor:"default", fontFamily:font, display:"flex", alignItems:"center", gap:12, marginBottom:24}}>
          ...
          <div style={{textAlign:"left"}}>
            ...
            <p style={{margin:0, fontSize: 16, color:T.grey500}}>재고 목록 → [발주] 버튼으로 담을 수 있어요</p>
          </div>
        </button>
      ) : (
        <>
                  ...
                  <div style={{padding:"18px 20px"}}>
                    <div style={{display:"flex", alignItems:"flex-start", gap:10, marginBottom:10}}>
                      ...
                      <div style={{flex:1, minWidth:0}}>
                        ...
                        <p style={{margin:"2px 0 0", fontSize: 16, color:T.grey500}}>현재 {item.current_qty}{item.unit} · 최소 {item.min_qty}{item.unit}</p>
                        {row.note && <p style={{margin:"3px 0 0", fontSize: 16, color:T.grey600, fontStyle:"italic"}}>"{row.note}"</p>}
                      </div>
                    ...
                    <div style={{display:"flex", alignItems:"center", gap:10}}>
                      ...
                      <p style={{margin:0, fontSize: 24, fontWeight:700, color:T.grey900, minWidth:40, textAlign:"center", fontVariantNumeric:"tabular-nums"}}>
                        {row.qty}<span style={{fontSize: 16, fontWeight:400, color:T.grey500}}>{item.unit}</span>
                      </p>
          ...
          <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", padding:"4px 4px 12px"}}>
            <p style={{margin:0, fontSize: 16, color:T.grey500}}>총 <span style={{fontWeight:700, color:T.grey800}}>{cart.length}개 품목</span> · <span style={{fontWeight:700, color:T.grey800}}>{totalQty}개</span></p>
            <button onClick={clearCart} style={{border:"none", background:"none", cursor:"pointer", fontSize: 16, color:T.grey400, fontFamily:font, textDecoration:"underline"}}>전체 비우기</button>
          </div>

          <button onClick={submitCart} style={{width:"100%", padding:"16px 0", borderRadius:9999, border:"none", cursor:"pointer", fontFamily:font, fontSize: 20, fontWeight:700, color:T.white, background:T.blue500, display:"flex", alignItems:"center", justifyContent:"center", gap:8, marginBottom:24}}>
      ...
      <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:12}}>
        ...
        <p style={{margin:0, fontSize: 20, fontWeight:700, color:T.grey900}}>내 발주 현황</p>
        {myOrders.length > 0 && <span style={{fontSize: 16, color:T.grey500}}>({myOrders.length})</span>}
      </div>

      {myOrders.length === 0 ? (
        <Card style={{padding:"24px 20px", textAlign:"center"}}>
          <p style={{margin:0, fontSize: 16, color:T.grey500}}>아직 요청한 발주가 없어요</p>
        </Card>
      ) : (
        <Card>
          {myOrders.map((o, i) => {
